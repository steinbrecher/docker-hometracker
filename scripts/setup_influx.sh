...
influx -ssl -unsafeSsl -host '127.0.0.1' -execute "CREATE USER ${INFLUXDB_ADMIN_USER} WITH PASSWORD '${INFLUXDB_ADMIN_PASSWORD}' WITH ALL PRIVILEGES"
...
influx -ssl -unsafeSsl -host '127.0.0.1' -execute "CREATE USER ${INFLUXDB_USER} WITH PASSWORD '${INFLUXDB_PASSWORD}' WITH ALL PRIVILEGES"
influx -ssl -unsafeSsl -host '127.0.0.1' -execute "CREATE DATABASE ${INFLUXDB_DATABASE_NAME}"
